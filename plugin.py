...
import arrow
from six.moves.urllib.parse import urlencode, parse_qsl, urlparse, parse_qs, unquote
from kodi_six import xbmc, xbmcplugin
from slyguy import plugin, inputstream, gui, signals, log
...
def play_channel(reference, region=None, **kwargs):
    ...
    try:
        ...
        url = yo_up_decoded + 'index.m3u8?' + yo_pp_base64_decoded
    except Exception as e:
        log.debug("failed to get raw url for: {} ({}). Fallback to yo stream".format(url, e))
        # fix encoded query
        if '?' in url:
...
def playlist(output, **kwargs):
    region = settings.REGION.value
    data = _channels(region)  # assuming this returns a list
    ...
    with codecs.open(output, 'w', encoding='utf8') as f:
        ...
        for row in data['channels']:
            ...
            #f.write('\n')  # separate JSON from the next object / playlist entry

            url=row.get('stream', {}).get('url')
            if not url:
                continue

            f.write(u'\n#EXTINF:-1 channel-id="{id}" tvg-id="{id}" tvg-logo="{logo}" tvg-chno="{chanNo}" group-title="{reg}" , {name}\n{url}'.format(
                id='mjh'+'-'+row['slug']+'-'+region, 
                name=row['name'], 
                logo=row['image']['sizes']['w768'],
                url=row['stream']['url'],
                chanNo=row['id'],
                reg=region),
            )
